...
silhouette = [silhouette_score(X, model.labels_) for model in kmeans_per_k[1:]]
...
def plot_dbscan(dbscan, X, size, show_xlabels=True, show_ylabels=True):
    ...
    anomalies = X[anomalies_make]
    non_cores = X[non_core_make]
    
    plt.scatter(cores[:, 0], cores[:, 1], c=dbscan.labels_[core_make], marker='o', s=size, cmap="Paired")
    plt.scatter(cores[:, 0], cores[:, 1], marker='*', s=20, c=dbscan.labels_[core_make])
    plt.scatter(anomalies[:, 0], anomalies[:, 1], c="r", marker="x", s=100)
    plt.scatter(non_cores[:, 0], non_cores[:, 1], c=dbscan.labels_[non_core_make], marker=".")
    if show_xlabels:
        plt.xlabel("$x_1$")
    else:
        plt.tick_params(labelbottom=False)
