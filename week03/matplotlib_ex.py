...
plt.xlabel('Age')
plt.ylabel('Count')
plt.title('Distribution of Ages no the Titanic')
plt.grid(axis='y', linestyle='--', alpha=0.7)
...
# 히트맵 그리기
plt.metshow(correlation_matrix, cmap='PuRd_r')
plt.colorbar()
...
# Survived가 0이면 사망자을, 1이면 생존자를 나타냄
...
# 두 번째 서브플롯 설정 (막대 그래프)
axes[1].bar(x, y2, color='deeppink', alpha=0.7, width=0.7, label='Survived')
...
# 각 부모와 자녀의 수에 따른 생존자와 사망자 수 계산
parch_counts = titanic.groupby('parhc')['Survived'].value_counts().unstack().fillna(0)
print(parch_counts)
...
# .twins() 함수를 사용하여 두 번째 그래프와 축을 공유하는 새로운 Axes 객체 생성
ax2 = ax1.twinx()

# 두 번째 그래프 설정 (막대 그래프)
ax1.bar(x, y2, color='deeppink', alpha=0.7, width=0.7, label='Survived')
